...
router.get('/check-evaluation-status/:requestID', async (req, res) => {
  try {
    ...
    console.log("requestID: ", requestID, departmentName, cityName);
...
router.post("/new-evaluation", async (req, res) => {
  try {
    ...
    console.log("Inside /new-evaluation")
    ...
    if (selectedDepartment) {
      ...
      //find evaluation and update the ones bellow (not the req) 
...
async function updateCityScore(selectedCity, updatedDepartment,demographicsChanged) {
  try {
    ...
    const updatedDepartments = selectedCity.departments.map(department => {
      // let currentDepartmentID = department._id.toString()
      // let updatedDepartmentID = updatedDepartment._id.toString()
      // console.log("depID: ", currentDepartmentID, typeof(currentDepartmentID))
      // console.log("updID: ", updatedDepartmentID, typeof(updatedDepartmentID))
      if(department.name=== updatedDepartment.name && department.city===updatedDepartment.city){
        // if (department._id.toString() === updatedDepartment._id.toString()) {  //different cause one depObj and the other depObj of city
